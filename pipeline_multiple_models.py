...
def extract_after_anchor(text: str, anchor: str) -> str:
    """
    Return everything after the first `anchor`, **but stop** if we later see
    an 'Answer:' token or a double line-break.
    """
    if anchor not in text:
        return text.strip()

    res = text.split(anchor, 1)[1]          # keep tail
    # hard stops
    for stop_tok in ("Answer:", "\n\n", "\nAnswer"):
        if stop_tok in res:
            res = res.split(stop_tok, 1)[0]
    return res.strip()
